...
%; Title, Abstract, TOC
\title{Behaviors of Seguinian nonlocal curvature}
...
\begin{abstract}
  We study the Seguinian notion nonlocal curvature of a curve and show that to classical signed curvature can be recovered under an appropriate limit, akin to other nonlocal recovery results. It's also shown that Seguinian nonlocal curvature of a curve in a $k$ dimensional subspace is also lives in the same $k$ dimensional subspace, further showing parallels between Seguinian nonlocal and classical signed curvature.
\end{abstract}
...
Seguinian nonlocal curvature of curves was introduced by Seguin \cite{seguin:2020} who defined the $\sigma-length$ for $\sigma \in (0, 1)$ of a curve $\C$ relative to an open, bounded set with smooth boundary $\Omega \subset \R{n}$ by
$$
\mathrm{Len}_\sigma(\C, \Omega) := \int_{\D{\C}} r^{1-n-\sigma} \sup_{v \in \U{} \cap \braces{u}^\perp} \chi_\Omega(p + rv) d\haus{2n}{p, u, r}.
$$
It was shown the corresponding Euler-Lagrange is
$$
\parens{\int_{\aeven} - \int_{\aodd}} \frac{\pdot{a}{t(z)} b - \pdot{b}{t(z)}a}{r^{1+\sigma}} \, d\haus{2n-2}{a, b, r}, \qquad \textup{for all } z \in \C,
$$
which, since the minimization of classical length occurs when classical curvature vanishes, motivated the Seguinian notion of nonlocal curvature as
\begin{equation}
  \kappa_\sigma(z) := \parens{\int_{\aeven} - \int_{\aodd}} \frac{\pdot{a}{t(z)} b - \pdot{b}{t(z)}a}{r^{1+\sigma}} \, d\haus{2n-2}{a, b, r}, \qquad \textup{for all } z \in \C. \label{def:non-local-curv}
\end{equation}

This definition is analogous to Abatangelo and Valdinoci's \cite{abaval:2014} a notion of nonlocal mean curvature of an surface $E \subset \R{n}$
$$
H_\sigma(z) := \frac{1}{\omega_{n-2}} \int_{\R{n}} \frac{\chi_E - \chi_{E^c}}{\abs{z - x}^{n+\sigma}} dx \qquad \textup{for all } z \in \partial E.
$$

Abatangelo and Valdinoci showed classical mean curvature can be recovered under an appropriate limit, that is
$$
\lim_{\sigma \uparrow 1} \parens{1-\sigma} H_\sigma(z) = H(z).
$$
These two notions of curvature match, up to a multiplicative constant, in $\R{2}$. Consequently its known, with appropriate scaling classical signed curvature is recovered from Seguinian nonlocal curvature via the above limit relation in $\R{2}$.
%:
\subsection{Motivation}%;
The above result in $\R{2}$ motivates finding the appropriate scaling factor $\Lambda_{2,\sigma}$ to recover classical curvature. Further, it makes sense to extend this result to $\R{n}$ so that the signed curvature of any $C^2$ curve $\C \subset \R{n}$ with non-vanishing signed curvature can be recovered via
$$
\lim_{\sigma \uparrow 1} \parens{1-\sigma} \Lambda_{n,\sigma} \kappa_\sigma(z) = \kappa(z) \qquad \textup{for all } z \in \C.
$$
Motivated by the relationship between classical curvature of a curve $\C$ and the osculating circle at a point $z \in \C$ $\textup{osc}(z)$, we'll show Seguinian curvature abides by
$$
\lim_{\sigma \uparrow 1} \abs{\kappa_{\sigma}^{\textup{osc}}(z) - \kappa_{\sigma}^{\C}(z)} = 0.
$$
Using the calculation of Seguinian curvature of a circle of arbitrary radius we'll be able to define
$$
\Lambda_{n, \sigma} := \frac{2^{1/2} \pi^{n-1}}{\sigma R^\sigma} \frac{\G{1+\sigma/2} \G{(1-\sigma)/2}}{\G{(n+1)/2}\G{(n+\sigma)/2}\G{1-\sigma/2}}.
$$
Together these results will give us the limit relation stated above.

The calculation of the Seguinian curvature of a circle in $\R{n}$ will reveal a relationship to the Seguinian curvature of a circle in $\R{2}$. This motivates showing a curve in a $k$ dimensional subspace has Seguinian curvature spanned by the same $k$ dimensional subspace.
%:
\subsection{Notation}%;
...
\newpage
\section{Vanishing non-normal components of Seguinian curvature}%;
Classically it's trivial to show the signed curvature of a curve at a point $z$ is parallel to $n(z)$. Seguinian curvature the same property, which is easy to see in $\R{2}$ since
$$
  \dot{\ks(z)}{t} = \parens{\int_{\Ae} - \int_{\Ao}} \frac{\pdot{a}{t}\pdot{b}{t} - \pdot{b}{t}\pdot{a}{t}}{r^{1+\sigma}} \, d\haus{2}{a,b,r} = 0.
$$
The following result extends this to $\R{n}$.
\begin{lemma}\label{lem:circnparallel}%;
  For a curve $\C \in \R{n}$ $\ks(z) = \parens{n \otimes n} \ks(z)$.
\end{lemma}

\begin{proof}
  Just as in the 2D case it's trivial to see $\pdot{\ks(z)}{t} = 0$. Now for $m \in \U{\braces{n,t}^\perp}$ consider the transformation $R_m := I - 2(m \otimes m)$ and put $\Phi : \A(z) \to \A(z)$ given by
  $$
  \Phi(a, b, r) = (R_m a, R_m b, r).
  $$
  It's easy to see $\Phi$ is an isometric diffeomorphism of $\A(z)$. Fix the integrand of $\dot{\ks(z)}{m}$ by
  $$
    J(a, b, r) := \frac{\pdot{a}{m}\pdot{b}{m} - \pdot{b}{m}\pdot{a}{m}}{r^{1+\sigma}}
  $$
  and notice
  \begin{IEEEeqnarray*}{rCl}
    r^{1+\sigma} J(R_m a, R_m, b, r) &=& \pdot{R_m a}{t}\pdot{R_m b}{m} - \pdot{R_m b}{t}\pdot{R_m a}{m} \\
    ...
    &=& \parens{\pdot{a}{t}}\parens{-\pdot{b}{m}} - \parens{\pdot{b}{t}}\parens{-\pdot{a}{m}} \\
    &=& -\pdot{a}{t}\pdot{b}{m} + \pdot{b}{t}\pdot{a}{m} = -r^{1+\sigma} J(a, b, r).
  \end{IEEEeqnarray*}
  That is, $\Phi$ is an isometric diffeomorphism which the integrand is odd over, so that
  \begin{equation}
    \ks(z) \cdot m = \parens{\int_{\Ae} - \int_{\Ao}} \frac{\pdot{a}{m}\pdot{b}{m} - \pdot{b}{m}\pdot{a}{m}}{r^{1+\sigma}} \, d\haus{2}{a,b,r} = 0.
  \end{equation}
\end{proof}
%:
%:

\section{Seguinian curvature of circles}%;

To get a feel for the geometry of Seguinian curvature we calculate $\ks(z)$ of a circle of arbitrary radius. A circle's symmetry allows us to ignore which $z$ we're analyzing, i.e. $\ks := \ks(0) = \ks(z)$, so for the rest of this section we'll simplify notation and focus on $\ks$. Our initial result in $\R{2}$ illuminates the fundamental geometries behind the definition, and provides a result we can reuse in $\R{n}$.

\begin{theorem}\label{thm:circr2}%;
...
\noindent Now we're prepared to calculate the Seguinian curvature of a circle in $\R{n}$, with our strategy as follows:
\begin{enumerate}[label=\roman*.]
  \item Recognize the \hyperref[lem:pu]{similarity with the 2D case} and \hyperref[nd:def:psi]{slice the domain along each $u$} intersecting $\S := \lsp{t, n}$.
  \item \hyperref[nd:def:phi]{Slice the domain along each radius $r$}.
  \item Show the domain of the inner-most integral with $u, r$ fixed is \hyperref[nd:eq:5]{a pair of spheres}.
  \item \hyperref[nd:eq:11]{Evaluate} the final integrals.
\end{enumerate}

\begin{theorem}\label{thm:circrn}%;
  For a circle of radius $R$ in $\R{n}$
  $$
  \ks =
  \frac{\omega_{n-3}^2}{\sigma R^\sigma \sqrt{2}} \B{\frac{3}{2}, \frac{n-2}{2}} \B{\frac{\sigma+2}{2}, \frac{n-2}{2}} \B{\frac{1}{2}, \frac{1-\sigma}{2}}
  n(z).
  $$
\end{theorem}
\begin{proof}
  First notice that the number of times a disk intersects $\C$ is completely determined by the projection of that disk into $\lsp{t, n}$. This motivates using the co-area formula to slice the domain into the line segments $\D{ra, b, r} \cap \S$ where $\S := \lsp{t, n}$. To that end, put $\Psi : \mathcal{A}^+ \to \mathbb{R}^2$  so that
  $$
...
$$
  We can further rewrite this by noticing $\dot{\Prsbp}{a} = \dot{\parens{b_t t + b_n n}^\perp}{a} = \dot{\parens{b_n t - b_t n}}{a} = b_n a_t - b_t a_n$, and putting $p(b) = \frac{\Prsbp}{\Prsbpa}$, so that
\begin{equation}
  \Psi(a, b, r) = 2r \frac{\dot{\Prsbp}{a}}{\Prsbpa^2} \mathcal{P}_S b^\perp = 2r \pdot{p(b)}{a} p(b) = 2r \parens{p(b) \otimes p(b)}a. \label{nd:def:psi}
\end{equation}
With $\E{u} := \Psi^{-1}(\braces{u})$, and \ref{lem:co:psi} we see
$$
  \ks
...
$$
As noticed above $\C \in \S$ so $\chit{\A}{a,b,r}$ must only depend on where $\D{a,b,r} \cap \S$, i.e. $u$. Putting\footnote{$\Pu$ is notably the same as in the 2D case. This comes from the inherent geometry of the circle, i.e. this set includes all $u$ who point below the circle (i.e. $\pdot{u}{n} < 0$), or $u$ that lie within the interior of the circle, i.e. $\abs{u} < 2R\sin\theta$ where $\theta$ is the angle between $t$ and $u$.}
$$\Pu := \braces{ u \in \S \mid \pdot{u}{n} < 0 \textup{ or } \frac{\abs{u}}{2} < R\parens{\frac{u}{\abs{u}} \cdot n} }$$
we must have $\chit{\A}{a,b,r} = \chit{\Pu}{u}$ and so
\begin{equation}
  \ks
...
\end{equation}
From \eqref{lem:circnparallel} we known we can focus on $\ks \cdot n$. It's also true
$$
...
and since $u = \frac{2r}{\Prsbpa^2} \pdot{\Prsbp}{a} \Prsbp \implies \abs{\dot{\Prsbp}{a}} = \frac{\abs{u}\Prsbpa}{2r}$, $\sgn{\dot{\Prsbp}{a}} = \sgn{\dot{\Prsbp}{u}}$ we have
  ...
  \end{equation}
We can slice the domain of the inner integral along each $r$, i.e. for $\Phi : \E{u} \to \R{+}$ given by
\begin{equation}
  \Phi(a, b, r) = r, \label{nd:def:phi}
\end{equation}
\noindent put $\E{u, r} = \Phi^{-1}(\{r\})$, so that with the co-area calculation in \ref{lem:co:phi} we find
\begin{IEEEeqnarray}{ll}
  ...
  \, d\haus{2n-5}{a, b} \, dr \, d\haus{2}{u} \nonumber
\end{IEEEeqnarray}
Since the inner most integrand only depends on $b$ it's natural to slice on each $b$. Put $\Xi : \E{u, r} \to \Et{u, r}$ where $\Xi(a, b) = b$. With the co-area calculation from \ref{lem:co:xi}, after putting $\E{u, r, b} = \Xi^{-1}(b)$ and including the result from \ref{lem:measure:eurb}, the above integral becomes
\begin{IEEEeqnarray}{ll}
  &\frac{-1}{2\sqrt{2}}
  \int_{\mathbb{R}^2} \chit{\Pu}{u}
  \int_{\Phi(\E{u})} \frac{1}{r^{3+\sigma} \sqrt{2 - \frac{\abs{u}^2}{4r^2}} \sqrt{1-\frac{\abs{u}^2}{4r^2}}}
...
\end{IEEEeqnarray}
To understand $\zeta\parens{\Et{u,r}}$ notice $b \in \Et{u, r} \implies \dot{b}{t} > 0$, so
$$
...
\noindent Putting \eqref{nd:eq:9} back into \eqref{nd:eq:5} our calculation becomes
\begin{equation}
...
\noindent To evaluate the final integrals we recall $\E{u} = \Psi^{-1}\parens{\braces{u}}$ where $\Psi$ is given in \eqref{nd:def:psi}. By definition of $\Phi$ in \eqref{nd:def:phi} we have
$$
...
\end{IEEEeqnarray}
where in \eqref{nd:eq:12} we leveraged \ref{lem:pu}.
\end{proof}
%:

\noindent With the main calculation out of the way we're ready to show the first asymptotic relationship in $\R{n}$.
\begin{theorem} \label{thm:circlim}%;
  For a circle of radius $R$
  $$
    \lim_{\sigma \uparrow 1} \parens{1 - \sigma} \Lambda_{n, \sigma} \ks = \frac{1}{R} n
  $$
  where $n$ is the unit normal and
  \begin{equation}
    \Lambda_{n,\sigma} = TODO. \label{def:lambda}
  \end{equation}
  That is, classical curvature of a circle is recovered in the canonical nonlocal limit.
\end{theorem}
\begin{proof}
  By \eqref{lem:circnparallel} we know $\ks$ only has a normal component so our goal is to show $\ks \cdot n \rightsquigarrow \frac{1}{R}$.
